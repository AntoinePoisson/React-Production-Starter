...
/**
 * We check the data, not the rendered HTML. rootHead / localeHead are just
 * functions — we care about the tags, not how React serialised them.
 */

/** Fresh module graph each time — the origin is read once at import. */
...
describe('Content Security Policy', () => {
  it('should carry every directive the app needs and nothing more', () => {
    ...
    // Ignored in a meta tag, and some browsers log an error if you put it there anyway.
    expect(CONTENT_SECURITY_POLICY).not.toContain('frame-ancestors');
    // Belongs at the edge. WebKit doesn't exempt localhost, so this breaks
    // e2e / previews / a phone on the LAN. Chromium and Firefox hide it.
    expect(CONTENT_SECURITY_POLICY).not.toContain('upgrade-insecure-requests');
    // 'unsafe-inline' we have to live with. 'unsafe-eval' we don't.
