...
// Goes through the logger, so shipping them is a transport decision.
// Dynamic import — stay off the critical path.
...
/** `poor` is warn so it shows in prod. The rest stay info. */
export const reportVital = ({ name, value, rating }: VitalReport): void => {
  // Sub-ms precision is noise for a field metric.
...
export const startVitalsReporting = async (): Promise<void> => {
  ...
  try {
    ...
    // Leave started=true. One of these can throw after others already registered
    // (onINP with no `event` entry, etc.) and a retry would double-subscribe.
