...
// Locale routing against the real build: two separate documents on two URLs, no redirect
// between them, no JavaScript needed to move between them, and no navigation when there is.
...
test.describe('Locale routing', () => {
  ...
  for (const locale of LOCALES) {
    test(`should serve ${locale.name} at ${locale.path} with the right lang`, async ({ pageWithVitals }) => {
      ...
      // The pathname is compared exactly, slash included: `status()` reports the end of the
      // redirect chain, so a server that bounced `/fr` to `/fr/` would still read 200 here.
  ...
  test('should switch language without reloading the document', async ({ pageWithVitals }) => {
    ...
    // Still a real href: crawlers, cmd-click and no-JS need it. Only the plain click is intercepted.
    ...
    // The URL has to follow, or a reload lands back on English and a shared link is the wrong language.
  ...
  test('should keep the WebGL context across a language switch', async ({ pageWithVitals }, testInfo) => {
    ...
    // React replaces the canvas element on a remount, the browser on a navigation. A surviving
    // stamp means the same element, and so the same context, geometry and camera.
    await pageWithVitals.evaluate(() => {
    ...
    expect(stamped, 'the canvas was recreated — its context, camera and scene state went with it').toBe(true);
  });
  ...
  test('should not link the current locale to itself', async ({ pageWithVitals }) => {
    ...
    // A link to the page you are already on is noise for keyboard and screen-reader users.
  ...
  test('should ship the copy in the HTML rather than in the JavaScript', async ({ pageWithVitals, request }) => {
    // True on every build: the copy is server-rendered either way, so it paints before the
    // 3D bundle has downloaded.
    ...
    // No catalogue may reach the initial payload. A production build strips the source text
    // from `<Trans>`, so a chunk carrying readable UI copy means something imported a
    // catalogue statically. `useLocaleSwitch.ts` names both, but only through `import()`, never
    // a `<script src>`, and only loaded scripts are inspected here.
...
// `useLocaleSwitch` is an enhancement. With no JavaScript the anchor carries the whole feature
// on its own, which is also what a crawler does with it.
