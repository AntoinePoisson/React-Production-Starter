...
/**
 * Looks for keys, tokens, that kind of thing.
 *
 * Staged diff by default (pre-commit). --all in CI, because after checkout
 * the index equals HEAD and staged mode would scan nothing.
 *
 * CRITICAL / HIGH block. MEDIUM is just reported. Add `secrets-check:ignore` on a line to skip it.
 */

import { execFileSync } from 'child_process';
import { existsSync, readFileSync } from 'fs';

/** Skip lines with this marker. */
...
// Skip list. .github/workflows stays in — that's the only tracked place a CI
// token would show up. ${{ secrets.X }} doesn't trip the patterns ($ and { aren't in the class).
const SKIP_PATTERNS = [
  /pnpm-lock\.yaml$/,
  /package-lock\.json$/,
  /yarn\.lock$/,
  /\.lock$/,
  /scripts\/check-secrets\.mjs$/, // this file *is* the patterns
  /node_modules/,
  /\.git\//,
  // Vendored minified stuff. Draco alone trips the entropy check 16 times.
  /(^|\/)public\/assets\/models\/draco\//,
  // Anchored. Unanchored /out\// also matched src/checkout/, /build\// matched src/rebuild/.
  /(^|\/)dist\//,
  /(^|\/)build\//,
  /(^|\/)out\//,
  /(^|\/)coverage\//
...
const SECRET_PATTERNS = [
  ...
  {
    name: 'Generic Token',
    // No leading \b — underscore is a word char, so \bapi misses CLOUDFLARE_API_TOKEN.
...
function scanFile(filePath, source, { isDiff } = { isDiff: true }) {
  ...
  // Diff mode: only added lines. `+++ b/path` is a header, not a change.
